...
class DexScreener:
    ...
    def get_token_market_data(self, ticker):
        """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if market_cap >= config.MARKET_CAP_FILTER:
                    self.logger.warning(
                        f"Most liquid pair for {ticker} has market cap (${market_cap:,.2f}) > ${config.MARKET_CAP_FILTER} - skipping"
                    )
                    return None
