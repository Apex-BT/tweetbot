...
def get_coinmarketcap_standard_price(ticker):
    ...
    try:
        ...
        if response.status_code == 200 and data.get("data"):
            ...
            logger.warning(f"Invalid request for {ticker}: {data.get('status', {}).get('error_message')}")
            return None
        else:
            logger.error(f"CMC API error ({response.status_code}): {data.get('status', {}).get('error_message')}")
            return None

    except Exception as e:
        logger.error(f"Error getting CoinMarketCap standard price for {ticker}: {str(e)}")
        return None
...
def get_historical_price(ticker, timestamp, contract_address=None, network_id=None, network_slug=None):
    ...
    # Fallback to CoinMarketCap standard API
    logger.info(f"Falling back to CoinMarketCap standard API for historical price of {ticker}")
    return get_coinmarketcap_standard_historical_price(ticker, timestamp)

def get_coinmarketcap_dex_historical_price(ticker, timestamp, contract_address=None, network_id=None, network_slug=None):
...
def get_coinmarketcap_standard_historical_price(ticker, timestamp):
    ...
    try:
        ...
        logger.error(f"Error getting CMC standard historical price for {ticker}: {str(e)}")
        return None
