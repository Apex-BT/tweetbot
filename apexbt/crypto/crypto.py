    ...
    try:
        ...
        if response.status_code == 200:
            ...
            sorted_pairs = sorted(
                matching_pairs,
                key=lambda x: (
                    float(x.get('liquidity', {}).get('usd', 0) or 0),  # Prioritize liquidity
            ...
            if market_cap >= MARKET_CAP_FILTER:
                logger.warning(f"Most liquid pair for {ticker} has market cap (${market_cap:,.2f}) > ${MARKET_CAP_FILTER} - skipping")
                return None
